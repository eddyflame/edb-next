...
- **待完善方案**：
  1. **线程独立挂起/解冻契约**：在 `ThreadsView` 中支持右键针对特定线程标记为 Frozen（冻结），通过 `SIGSTOP` 信号或在事件循环处理逻辑中显式剥离其被唤醒权限；
  2. **隔离单步步进 (Single-Thread Stepping)**：在调试存在数据竞争或死锁隐患的复杂多线程服务时，仅驱动当前焦点线程执行单步，其他所有工作线程保持冻结，防止执行现场被并发破坏。

### 4.10 动态内存特征差分扫描器 (Memory Scanner / 数据变动检索)
- **当前状态**：当前 `PatternSearcher` 仅支持静态特征码（含通配符掩码）在只读/可执行段的一次性检索，无法应对逆向工程中寻找动态内存变量、加密密钥或偏移定位的高频需求。
- **待完善方案**：
  1. **CheatEngine 级多轮差分内存扫描**：遍历 `/proc/<pid>/maps` 中所有读写权限段（`rw-p`），支持第 1 轮全内存初值快速检索（数值、文本、Hex）；
  2. **动态收敛判定**：在目标继续运行一段后，支持“数值变大”、“数值变小”、“数值变动”、“数值未变”的多轮差分匹配，以极高效率从数百万内存地址中精准收敛定位关键敏感变量。
...
| :--- | :---: | :---: | :---: | :--- |
| **4.3 C++ 符号反混淆 (Demangling)** | ★★★★★ | 极低 (Low) | **P0 (最优先)** | **当前即刻推进**。无外部依赖，立刻使反汇编、调用栈与符号表达到工业级可读性。 |
| **4.5 细粒度硬件读写监视点 UI** | ★★★★☆ | 极低 (Low) | **P0 (最优先)** | **当前即刻推进**。底层 DR0~DR7 已完整就绪，仅需在内存 HexDump 右键菜单补齐下断交互。 |
| **4.2 高级反反调试与隐蔽断点 (Page-Guard)** | ★★★★★ | 中等 (Medium) | **P1 (核心壁垒)** | **近期重点攻坚**。填补 Linux 平台反反调试工具空白，解决恶意样本 CRC 自校验与 `TracerPid` 检测。 |
| **4.7 动态库加载自动拦截 (`_r_debug`)** | ★★★★☆ | 中等 (Medium) | **P1 (核心壁垒)** | **近期重点攻坚**。解决动态 `dlopen()` 模块符号丢失问题，对齐 GDB 核心基础设施。 |
| **4.8 断点绑定 Python/Lua 脚本打桩** | ★★★★☆ | 中等 (Medium) | **P1 (核心壁垒)** | **近期重点攻坚**。盘活新引入的双脚本引擎，实现无需重启、无需源码的超高速无感 Hook。 |

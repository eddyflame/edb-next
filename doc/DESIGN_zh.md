...
6. **AVX2 向量化多线程特征码内存搜索 (PatternSearcher)**：
   - 支持形如 `f3 0f ?? fa` 的通配符掩码搜寻，首创首尾非通配双锚点 AVX2 SIMD 掩码过滤算法，256 位宽并行排除 32 字节候选，吞吐量达到 ~2.87 GB/s（38.27x 加速）；
   - 以 2MB 步长流式分块扫描彻底消除历史版本 16MB 内存截断 Bug，恒定 2MB 内存占用轻松检索 GB 级大内存空间；
   - 支持多段并行并发工作窃取调度与原子早停，老旧 CPU 自动透明降级为标量算法。
